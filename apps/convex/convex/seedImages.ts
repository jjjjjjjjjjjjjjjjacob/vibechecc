...
function getImageForVibe(vibeTitle: string, vibeDescription: string): string {
  ...
  return publicDomainImages[
    Math.floor(Math.random() * publicDomainImages.length)
  ];
}
  ...
  handler: async (
    ...
    for (let i = 0; i < maxToUpdate; i++) {
      ...
      const storageId = await ctx.runAction(
        api.seedImages.downloadAndStoreImage as any,
        { url: imageUrl } as any
      );
...
export const getVibesWithoutImages = internalQuery({
  handler: async (
    ctx
  ): Promise<import('./_generated/dataModel').Doc<'vibes'>[]> => {
