...
export function parseSearchQuery(query: string): ParsedQuery {
  ...
  const terms = processedQuery.trim().split(/\s+/).filter(term => term.length > 0);
...
export function matchesParsedQuery(
  ...
  if (parsedQuery.filters.user && options.username) {
    if (options.username.toLowerCase() !== parsedQuery.filters.user.toLowerCase()) {
      return false;
    }
  }
  ...
  if (options.rating !== undefined) {
    if (parsedQuery.filters.minRating && options.rating < parsedQuery.filters.minRating) {
      return false;
    }
    if (parsedQuery.filters.maxRating && options.rating > parsedQuery.filters.maxRating) {
      return false;
    }
  }
...
export function describeSearch(parsedQuery: ParsedQuery): string {
  ...
  if (parsedQuery.filters.minRating || parsedQuery.filters.maxRating) {
    if (parsedQuery.filters.minRating && parsedQuery.filters.maxRating) {
      parts.push(`rating: ${parsedQuery.filters.minRating}-${parsedQuery.filters.maxRating}`);
