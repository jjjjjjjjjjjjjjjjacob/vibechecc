...
export function UserInterestsSection({
  ...
  const availableTags = React.useMemo(() => {
    ...
    if (derivedInterests) {
      derivedInterests.forEach((tag) => tagSet.add(tag));
    }

    // Add all available tags
    if (allTags) {
      allTags.forEach((tagData) => tagSet.add(tagData.tag));
    }
          ...
          >
            ...
            {derivedInterests && derivedInterests.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {derivedInterests.slice(0, 8).map((interest) => (
                    <Badge
                      key={interest}
                      variant={
