import { Heart, MessageCircle, Sparkles, UserPlus, Grid3X3 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { cn } from '@/utils/tailwind-utils';

export type NotificationFilterType = 'all' | 'likes' | 'comments' | 'mentions' | 'followers';
...
export function NotificationFilters({ 
  ...
  return (
    <div className="border-border/50 flex overflow-x-auto border-b">
      <div className="flex min-w-full space-x-1 p-1">
        {filterConfig.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <Button
              ...
              size="sm"
              className={cn(
                'hover:bg-muted/50 relative min-w-0 flex-shrink-0 justify-start gap-2 text-xs lowercase transition-colors',
