...
import { useToggleRatingBoostMutation, useToggleRatingDampenMutation } from '@/queries';
...
export function RatingDootButton({
  ...
  const [optimisticVoteStatus, setOptimisticVoteStatus] = React.useState(voteStatus);
  ...
  const handleBoost = async (e: React.MouseEvent) => {
    ...
    if (wasAlreadyBoosted) {
      // Unboost
      setOptimisticVoteStatus({ voteType: null, boosted: false, dampened: false });
      setOptimisticScore(prev => prev - 1);
    } else {
      // Boost (might be switching from dampen)
      setOptimisticVoteStatus({ voteType: 'boost', boosted: true, dampened: false });
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'failed to boost review';
  ...
  const handleDampen = async (e: React.MouseEvent) => {
    ...
    if (wasAlreadyDampened) {
      ...
      setOptimisticVoteStatus({ voteType: 'dampen', boosted: false, dampened: true });
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'failed to dampen review';
      toast.error(errorMessage, {
        duration: 4000,
        icon: '❌',
      });
    ...
    <div className={cn('flex items-center gap-1', className)}>
      ...
      <Button
        ...
        className={cn(
          ...
          optimisticVoteStatus.boosted && 'text-green-600 hover:text-green-600 dark:text-green-400',
          !optimisticVoteStatus.boosted && 'text-muted-foreground hover:text-green-600 dark:hover:text-green-400'
        )}
        aria-label={optimisticVoteStatus.boosted ? 'Unboost review' : 'Boost review'}
      >
        <ChevronUp 
          className={cn(
      ...
      <Button
        ...
        className={cn(
          'h-8 w-8 p-0 transition-all',
          optimisticVoteStatus.dampened && 'text-red-600 hover:text-red-600 dark:text-red-400',
          !optimisticVoteStatus.dampened && 'text-muted-foreground hover:text-red-600 dark:hover:text-red-400'
        )}
        aria-label={optimisticVoteStatus.dampened ? 'Undampen review' : 'Dampen review'}
      >
        <ChevronDown 
          className={cn(
            'h-4 w-4 transition-all',
