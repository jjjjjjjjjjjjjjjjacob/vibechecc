...
export const gradientPresets = [
  ...
  { name: 'ocean', from: '#4CA1AF', to: '#2C3E50', direction: 'to-br' },
  { name: 'forest', from: '#134E5E', to: '#71B280', direction: 'to-br' },
...
export function generateRandomGradient() {
  const colors = [
    '#FF6B6B', '#4CA1AF', '#667EEA', '#00B09B', '#EB3349',
    '#FF6FD8', '#FFE000', '#FF6B9D', '#FFA500', '#2C3E50',
    '#134E5E', '#764BA2', '#FFEAA7', '#96C93D', '#F45C43',
    '#232526', '#3813C2', '#FF6C00', '#FFC600', '#71B280',
  ];
...
export function generateGradientStyle(from?: string, to?: string, direction?: string) {
  if (!from || !to) {
    return '';
  }
...
export function generateGradientClasses(from?: string, to?: string, direction?: string) {
  if (!from || !to) {
    return '';
  }
...
export function isLightGradient(from: string, to: string) {
  ...
  // Calculate luminance
  const fromLuminance = (0.299 * fromRgb.r + 0.587 * fromRgb.g + 0.114 * fromRgb.b) / 255;
  const toLuminance = (0.299 * toRgb.r + 0.587 * toRgb.g + 0.114 * toRgb.b) / 255;
