...
export function generateSEOTags(config: SEOConfig): MetaTag[] {
  const tags: MetaTag[] = [
    ...
    { name: 'twitter:creator', content: config.author ? `@${config.author}` : SITE_CONFIG.creator },
...
// Utility functions for common SEO configurations
export const seoConfigs = {
  home: (): SEOConfig => ({
    title: `${SITE_CONFIG.name} - ${SITE_CONFIG.description}`,
    description: SITE_CONFIG.description,
    keywords: ['vibes', 'social', 'rating', 'community', 'sharing', 'discover'],
    type: 'website',
    url: SITE_CONFIG.url,
  }),

  vibe: (vibe: { title: string; description: string; tags?: string[]; createdBy?: { username: string }; image?: string; createdAt: string }): SEOConfig => ({
  ...
  profile: (user: { username: string; bio?: string; first_name?: string; last_name?: string }): SEOConfig => {
  ...
  tag: (tag: string, vibeCount?: number): SEOConfig => ({
    ...
    description: vibeCount !== undefined
...
export function generatePageTitle(title: string, includePrefix = true): string {
  ...
  return title.includes(SITE_CONFIG.name) ? title : `${title} - ${SITE_CONFIG.name}`;
}
