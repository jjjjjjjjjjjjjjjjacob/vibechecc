...
>(({ className, children, setTabs }, ref) => {
  ...
  React.useEffect(() => {
    ...
    React.Children.forEach(children, (child) => {
      if (React.isValidElement(child) && child.type === TabsDraggableTrigger) {
  ...
  const handleIndicatorDragStart = (e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  const handleIndicatorDrag = React.useCallback(
    ...
    [isDragging, tabPositions, activeTabIndex]
  );
...
>(({ value, className, style, children }, ref) => {
  const context = React.useContext(TabsDraggableContext);
  const { activeTabIndex, tabs } = context;
...
>(({ className, style, children }, _ref) => {
  const context = React.useContext(TabsDraggableContext);
  const { activeTabIndex, setActiveTabIndex, tabs, onTabChange } = context;
  ...
  // Calculate positions
  const contentTransform = `translateX(${-activeTabIndex * containerWidth + dragOffset}px)`;
