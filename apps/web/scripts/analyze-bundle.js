...
function analyzeFontFiles() {
  ...
  fontFiles.forEach((file) => {
    const isOptimized =
      file.name.includes('core') || file.name.includes('extended');
  ...
  const hasOriginalEmoji = fontFiles.some((f) =>
    f.name.includes('NotoColorEmoji-Regular')
  );
  const hasSubsetEmoji = fontFiles.some(
    (f) => f.name.includes('core') || f.name.includes('extended')
  );
