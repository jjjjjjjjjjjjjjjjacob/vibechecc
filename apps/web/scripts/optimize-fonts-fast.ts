...
/**
 * Compose a small CSS file that references the pre-optimized fonts.
 * The heavier subsetting happens in a separate Python script, so here
 * we only generate basic @font-face rules for critical fonts.
 */
...
/**
 * Entry point for the script. Creates the output directory and writes
 * a minimal CSS file pointing to the pre-optimized fonts.
 */
async function main() {
...
// Kick off the optimization and exit with non-zero code on failure
main().catch((_e) => {
  // console.error(e); // Logging disabled for speed
  process.exit(1);
});
