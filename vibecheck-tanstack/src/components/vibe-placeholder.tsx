...
export function VibePlaceholder({ title, variant = "default", className }: VibePlaceholderProps) {
  ...
  const getGradientClasses = () => {
    const gradients = resolvedTheme === "dark" ? darkModeGradients : lightModeGradients
    const baseGradient = gradients[colorIndex]

    switch (variant) {
      case "wavy":
        return cn(
          resolvedTheme === "dark" ? darkModeGradients[(colorIndex + 1) % 6] : lightModeGradients[(colorIndex + 1) % 6],
        )
      case "gradient":
        return cn(
          resolvedTheme === "dark"
  ...
  if (!mounted) {
    return (
      <div className={cn("relative w-full h-full bg-muted flex items-center justify-center", className)}>
        {title && <div className="absolute inset-0 flex items-center justify-center p-4 text-center opacity-0"></div>}
      </div>
    )
  }
